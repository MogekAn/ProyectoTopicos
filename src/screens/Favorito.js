...
const Favorito = () => {
  ...
  // Estado para almacenar los colores del icono de calendario
  const [calendarColors, setCalendarColors] = useState(
    eventosFavoritos.reduce((acc, evento) => {
      acc[evento.id] = "blue.500"; // Inicializamos todos los colores con "blue.500"
      return acc;
    }, {})
  );

  // Función para cambiar el color del calendario de un evento específico
  const toggleCalendarColor = (id) => {
    setCalendarColors((prevColors) => ({
      ...prevColors,
      [id]: prevColors[id] === "blue.500" ? "yellow.500" : "blue.500", // Alternamos entre azul y amarillo
    }));
  };

  // Función para eliminar un evento de la lista de favoritos
  const deleteEvent = (id) => {
    setEventosFavoritos(eventosFavoritos.filter(evento => evento.id !== id));
  };
          ...
          {eventosFavoritos.map((evento, index) => (
            ...
            >
              ...
              <Icon
                ...
                name="calendar"
                size="lg"
                color={calendarColors[evento.id]}  // Usamos el estado para cambiar el color del calendario específico
                position="absolute"
                bottom={10}
                left={4}
                onPress={() => toggleCalendarColor(evento.id)} // Llamamos a la función con el ID del evento
              />
              <Icon
                as={FontAwesome}
                name="trash"
                ...
                bottom={10}
                right={4}
                onPress={() => deleteEvent(evento.id)} // Llamamos a la función de eliminación
              />
              ...
              <Button
                ...
                onPress={() => {
                  if (evento.id === 3) {
                    navigation.navigate('Market', { event: evento });
                  } else {
                    navigation.navigate('EventoDetalle', { event: evento });
                  }
                  if (evento.id === 7) {
